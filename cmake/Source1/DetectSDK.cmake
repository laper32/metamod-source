...
# But wee need to configure with the case that: We are not using vendor.
if (NOT HL2SDK_USE_VENDOR)
    #
    # Using a trick: Our SDK path format is hl2sdk-*, and with the specified words(see above)
    # This means that we can identically treat that the environment variable is same as the folder name,
    # as the standard.
    #
    # So that we can directly make it upper, so that we can call it directly.
    #
    string(TOUPPER "${HL2SDK_FOLDER}" HL2SDK_FOLDER)

    # Then, here you are.
    if (DEFINED ENV{${HL2SDK_FOLDER}})
        set(HL2SDK_PATH_STRING $ENV{${HL2SDK_FOLDER}})
    else()
        message(FATAL_ERROR "You want to use environment variable but it does not exist. Check environment variable: ${HL2SDK_FOLDER}")
    endif()
else()
    # And here we go.
    set(HL2SDK_PATH_STRING "${CMAKE_SOURCE_DIR}/dependency/${HL2SDK_FOLDER}")
endif()
